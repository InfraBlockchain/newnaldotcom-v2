...
export function DeviceAccordion() {
  ...
  // TODO(asset): Replace ILLI and UFO placeholders when approved key visuals are provided.
    ...
    {devicesContent.devices.map((device, index) => (
      <article key={device.id} className={`${styles.panel} ${styles[device.id]} ${active === index ? styles.active : ""}`}>
        ...
        <div id={`device-${device.id}`} className={styles.content}>
          {device.id === "yali" ? <Image src="/images/yali-hero.png" alt="YALI AI Artist Companion in violet light" fill sizes="(max-width: 767px) 100vw, 80vw" priority /> : <div className={styles.placeholder} role="img" aria-label={`Placeholder for ${device.name} key visual`}><span>PHOTO · {device.name} key visual</span></div>}
