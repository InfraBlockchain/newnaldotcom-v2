...
│  Not another notification.   │  ◯ 디바 │     │      (행마다 리빌 스태거)
...
- **Aura Ring**: 디바이스 렌더(`yali-device.png`) 뒤에 conic-gradient 링(두께 ~2px + blur 글로우, `--lavender`→`--violet`). `--dur-ambient`로 scale 1→1.03 + opacity breathing — 페이지 유일의 상시 모션
- 부정 카피 3행: Instrument Sans 500, `--ink-soft`. 마무리 줄 `A daily companion…`만 `--ink` 강조
- `[옵션 — 확정 필요]` 구 기획안에 "홍보영상 썸네일+재생버튼(미연결 허용)" 지시가 있었음. 영상을 쓰기로 확정되면 디바이스 렌더 대신 16:9 썸네일 + 원형 재생 버튼(64px)으로 교체, Aura Ring은 썸네일 뒤 유지
