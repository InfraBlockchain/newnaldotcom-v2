...
- **톤 (2026-07-15 개정)**: 라이트 **모노톤** — 보라는 YALI 전용이므로 홈에서 유채색 금지. 세 카드 배경: 카드1(aios)=`--surface`, 카드2(Private Phone)=`--stage`(중립 다크), 카드3(Devices)=`--bg`보다 한 단계 어두운 중립 그레이 틴트. 본문의 `--violet` 언급은 `--ink`로 치환
