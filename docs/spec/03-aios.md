...
- **톤 (2026-07-15 개정)**: 라이트 **모노톤** — 이 페이지에서 유채색 전면 금지. 아래 본문에 남아 있는 `--violet`/`--lavender`/"라벤더" 언급은 전부 다음으로 치환해 읽는다: 액센트·액티브·글로우 → `--ink` 또는 중립 그레이 글로우(`--ink` 6% 투명), 타일 hover 보더 → `--ink`, 스펙트럼 끝점은 0=`--ink-faint` / 100=`--ink` 채움. 라이트, 텍스트 중심의 에디토리얼. 다이어그램 3개(스펙트럼·타임라인·Reverse Login)가 시각의 전부 — 전부 코드(SVG/CSS)로 구현, 이미지 금지
