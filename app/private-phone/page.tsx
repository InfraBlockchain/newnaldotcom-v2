...
import { ArrowUpIcon, ChevronLeftIcon, EllipsisHorizontalIcon } from "@heroicons/react/24/solid";
...
export default function PrivatePhonePage() {
  ...
  return <main id="main-content" className={styles.page} data-theme="private">
    ...
    <section className={`${styles.section} ${styles.leakSection}`}><div className={styles.container}><ChapterHead {...c.leak} /><div className={styles.leakPhones}>{[c.leak.a,c.leak.b].map((device,di) => <Reveal key={device.label} delay={di*120}><div className={`${styles.leakDevice} ${di === 0 ? styles.editedDevice : styles.originalDevice}`}><p className={styles.eyebrow}>{device.label}</p><PhoneSilhouette><div className={styles.chatScreen}><div className={styles.chatHeader}><ChevronLeftIcon className={styles.chatIcon} aria-hidden="true" /><div><strong>Private Contact</strong><small>123-ABC-5678</small></div><EllipsisHorizontalIcon className={styles.chatIcon} aria-hidden="true" /></div><p className={styles.chatTime}>Today · 8:42 PM</p><div className={styles.chatMessages}>{device.messages.map((message,mi) => <p key={message} className={`${styles.bubble} ${mi === 1 ? styles.sent : styles.received} ${mi===1 ? di === 0 ? styles.changed : styles.original : ""}`}>{message}</p>)}</div><p className={styles.leakCaption}>{device.caption}</p><div className={styles.chatComposer}><span>Message</span><span className={styles.sendIcon}><ArrowUpIcon aria-hidden="true" /></span></div></div></PhoneSilhouette></div></Reveal>)}</div><Reveal><p className={styles.displayClosing}>{c.leak.closing}</p></Reveal></div></section>

    <section className={`${styles.section} ${styles.panelSection}`}><div className={styles.container}><ChapterHead {...c.inbox} /><Reveal className={styles.networks}><div className={styles.hub}><p className={styles.eyebrow}>TRADITIONAL — ONE SHARED HUB</p><div className={styles.hubDiagram}><svg className={styles.hubConnections} viewBox="0 0 600 420" preserveAspectRatio="none" aria-hidden="true"><line x1="300" y1="210" x2="300" y2="52" /><line x1="300" y1="210" x2="548" y2="144" /><line x1="300" y1="210" x2="467" y2="364" /><line x1="300" y1="210" x2="138" y2="364" /><line x1="300" y1="210" x2="72" y2="144" /><line x1="300" y1="210" x2="410" y2="32" /><line x1="300" y1="210" x2="570" y2="225" /><line x1="300" y1="210" x2="395" y2="392" /><line x1="300" y1="210" x2="205" y2="392" /><line x1="300" y1="210" x2="30" y2="225" /></svg><span className={styles.you}>You</span>{["Family","Ad","Spam","Unknown","Friend"].map((x,i)=><span key={x} className={i>0&&i<4?styles.unwanted:""}>{x}</span>)}</div><p>One line, everyone sits on it.</p></div><div className={styles.wires}><p className={styles.eyebrow}>NEWNAL — SEPARATE PRIVATE WIRES</p><div className={styles.wireHeader}><span>YOU</span><span>PRIVATE NUMBER</span><span>CONTACT</span></div>{wires.map(([n,name])=><div className={styles.wire} key={n}><span>You</span><NumberChip>{n}</NumberChip><span>→ {name}</span></div>)}<p>Five wires. Five numbers. Nothing shared.</p></div></Reveal></div></section>
