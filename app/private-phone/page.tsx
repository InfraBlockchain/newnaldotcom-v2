...
export default function PrivatePhonePage() {
  ...
  return <main id="main-content" className={styles.page} data-theme="private">
    ...
    <section className={styles.section}><div className={styles.container}><ChapterHead {...c.leak} /><div className={styles.leakPhones}>{[c.leak.a,c.leak.b].map((device,di) => <Reveal key={device.label} delay={di*120}><div className={styles.leakDevice}><p className={styles.eyebrow}>{device.label}</p><PhoneSilhouette>{device.messages.map((message,mi) => <p key={message} className={`${styles.bubble} ${mi===1 ? styles.changed : ""}`}>{message}</p>)}</PhoneSilhouette><p className={styles.leakCaption}>{device.caption}</p></div></Reveal>)}</div><Reveal><p className={styles.displayClosing}>{c.leak.closing}</p></Reveal></div></section>
